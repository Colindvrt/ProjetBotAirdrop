...
# --- SECTION 2 : LA "PAGE" PARAMÈTRES (CORRIGÉ) ---
class SettingsPage(ttk.Frame):
    def __init__(self, parent, app_controller=None): 
        ...
        self.create_platform_frame("Extended", self.api_keys["extended"])
    ...
    def create_platform_frame(self, platform_name, key_vars):
        """Crée une boîte pour entrer les clés d'une plateforme (style CEX)."""
    ...
    def create_hyperliquid_frame(self):
        """Cadre spécifique pour Hyperliquid."""
    ...
    def create_lighter_frame(self):
        """Cadre spécifique pour Lighter (basé sur tes découvertes)."""
        ...
        try:
            if os.path.exists(CONFIG_FILE):
                ...
                if "extended" in config_data:
                    self.api_keys["extended"]["key"].set(config_data["extended"].get("key", ""))
                    self.api_keys["extended"]["secret"].set(config_data["extended"].get("secret", ""))
    ...
    def save_config(self):
        ...
        try:
            config_data = {
                ...
                "extended": {
                    "key": self.api_keys["extended"]["key"].get(),
                    "secret": self.api_keys["extended"]["secret"].get()
                }
            }
...
# (MODIFIÉE pour la nouvelle logique de pages .grid() / .tkraise())
...
class FundingScannerApp:
    def __init__(self, root):
        ...
        self.pages = {}
        # Crée les 5 pages au démarrage et les empile
        ...
        # Stocke une référence vers la page de funding pour la mise à jour
    ...
    def show_page(self, PageClass):
        ...
        if PageClass in self.pages:
            ...
            page.tkraise() # C'est la commande magique
    ...
    def _scan_thread(self, filters, callback):
        all_data = []
        try:
            # --- MODIFIÉ : Utilise les nouveaux noms de fonction ---
